...
local bytecommunity = "34ByTe Community"
...
Citizen.CreateThread(
	function()
		...
		while Enabled do
			...
			if LynxEvo.IsMenuOpened("LynxX") then
				drawDescription("Welcome back "..pisello.." !", 0.5, 0.5)
	...
	elseif LynxEvo.IsMenuOpened("ESXJobs2") then
		drawDescription("Useful ESX Triggers for player", 0.46, 0.46)
	...
	if mhaonn then
		...
		elseif LynxEvo.IsMenuOpened("GCT") then
			drawDescription("Global cars fuckery", 0.34, 0.34)
...
--LUA MENUS
		LynxEvo.Display()
			...
			elseif LynxEvo.IsMenuOpened("ESXJobs") then
				drawDescription("ESX Triggers for jobs", 0.46, 0.46)
			...
			elseif LynxEvo.IsMenuOpened("ESXBoss") then
				drawDescription("ESX Triggers for the boss menus", 0.46, 0.46)
...
elseif LynxEvo.CheckBox("~h~~r~DOT ~s~Crosshair", crosshairc2, function (enabled) crosshair = false crosshairc = false crosshairc2 = enabled end) then
	...
	elseif LynxEvo.IsMenuOpened("crds") then
		...
		if LynxEvo.Button("~h~~p~#~s~ nit34byte~r~#~r~0977 ~p~DEV") then
